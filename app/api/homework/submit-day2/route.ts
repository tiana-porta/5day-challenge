...
import { appendDay2Submission } from "@/lib/google-sheets";

export const dynamic = "force-dynamic";

interface Day2SubmissionBody {
  username: string;
  email: string;
  day: number;
  market: string;
...
export async function POST(request: NextRequest) {
  try {
    console.log("📝 POST /api/homework/submit-day2 - Processing submission...");

    const body: Day2SubmissionBody = await request.json();
    const { username, email, day, market, whyProfitable, problem, desiredOutcome, researchLink, notes } = body;
    ...
    if (!market || market.length < 3) {
      errors.market = "Please describe your market";
    }

    if (!whyProfitable || whyProfitable.length < 10) {
      errors.whyProfitable = "Please explain why this market is profitable";
    }
    ...
    console.log("✅ Day 2 homework submission successful for:", username);
    return NextResponse.json({
      success: true,
