...
export function SocialsForm({ dayNumber }: SocialsFormProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.profileLink || formData.profileLink.length < 5) {
      newErrors.profileLink = 'Please paste your profile/socials link'
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/homework/submit-day5', {
        method: 'POST',
    ...
    return (
      <HomeworkCard>
        <div className="text-center mb-8">
          <div className="inline-block px-3 py-1 rounded-full bg-green-500/20 text-green-400 text-sm font-semibold mb-4">
            Challenge Complete!
          </div>
          ...
          <p className="text-primary/60 mb-8">
            You've completed the Whop University 5 Day Challenge!
          </p>
        </div>
        <Diploma username={submittedUsername} />
      </HomeworkCard>
    )
  }
  ...
  return (
    <HomeworkCard>
      <div className="text-center mb-8">
        ...
        <h2 className="text-2xl md:text-3xl font-bold text-primary mb-2">Profile Funnel</h2>
        <p className="text-primary/60">Submit your socials/profile link</p>
      </div>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 rounded-xl bg-dark-lighter border-2 border-primary/10 text-primary placeholder-primary/30 focus:border-accent focus:outline-none transition-colors"
            placeholder="Paste your profile funnel or socials link"
          />
          {errors.profileLink && (
            <p className="mt-1 text-sm text-red-400">{errors.profileLink}</p>
          )}
        </div>
