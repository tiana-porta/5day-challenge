...
interface OnePageDocFormProps {
  dayNumber: number
}

export function OnePageDocForm({ dayNumber }: OnePageDocFormProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.docLink || formData.docLink.length < 5) {
      newErrors.docLink = 'Please paste your one-page doc link'
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/homework/submit-day3', {
        method: 'POST',
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-3 rounded-xl bg-dark-lighter border-2 border-primary/10 text-primary placeholder-primary/30 focus:border-accent focus:outline-none transition-colors"
            placeholder="Paste your Google Doc, Notion, or other link here"
          />
          {errors.docLink && (
            <p className="mt-1 text-sm text-red-400">{errors.docLink}</p>
          )}
        </div>
